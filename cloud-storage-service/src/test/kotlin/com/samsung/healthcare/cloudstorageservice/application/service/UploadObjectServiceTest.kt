...
internal class UploadObjectServiceTest {
    private val signedUrlProperties = SignedUrlProperties(120L)
    private val uploadObjectPort = mockk<UploadObjectPort>()
    private val uploadObjectService = UploadObjectService(signedUrlProperties, uploadObjectPort)
    ...
    fun `getSignedUrl should work properly`() {
        ...
        every {
            uploadObjectPort.getUploadSignedUrl("$projectId/$objectName", signedUrlDuration)
        } returns URL("https://test")

        StepVerifier.create(
            uploadObjectService.getSignedUrl(projectId, objectName)
        )
            .expectNextMatches {
                it == URL("https://test")
            }
    ...
    fun `getSignedUrl should throw IllegalArgumentException when projectId was empty`(projectId: String) {
        assertThrows<IllegalArgumentException> {
            uploadObjectService.getSignedUrl(projectId, "test-name")
        }
    }
    ...
    fun `getParticipantSignedUrl should work properly`() {
        ...
        StepVerifier.create(
            uploadObjectService.getParticipantSignedUrl(projectId, objectName)
        )
            .expectNextMatches {
                it == URL("https://test")
            }
    ...
    fun `getParticipantSignedUrl should throw IllegalArgumentException when projectId was empty`(projectId: String) {
        assertThrows<IllegalArgumentException> {
            uploadObjectService.getParticipantSignedUrl(projectId, "test-name")
        }
    }
